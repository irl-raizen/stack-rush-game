...
export function AuthForm({ mode }: { mode: "sign-in" | "sign-up" }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(email)) { setError("Enter a valid email address."); setPending(false); return }
    try {
      if (mode === "sign-in") await signOutExistingSession()
