...
type Room = { id: string; roomCode: string; mode: string; status: string; hostScore: number; guestScore: number; hostReady?: boolean; guestReady?: boolean; hostUserId?: string | null; guestUserId?: string | null; startAt?: string | Date | null }
...
export function MultiplayerScreen({ onBack, playerName, onMatchStart }: { onBack: () => void; playerName: string; onMatchStart: (roomId: string) => void }) {
  ...
  useEffect(() => { if (!room || room.status !== "countdown" || !room.startAt) return; const remaining = new Date(room.startAt).getTime() - Date.now(); const timer = window.setTimeout(() => onMatchStart(room.id), Math.max(0, remaining)); return () => window.clearTimeout(timer) }, [room?.status, room?.startAt, room?.id, onMatchStart])
