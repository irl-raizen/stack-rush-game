...
export default async function ChallengePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="flex min-h-screen items-center justify-center bg-background p-6 text-foreground"><div className="w-full max-w-md rounded-3xl border border-border bg-card p-8 text-center"><p className="font-mono text-xs uppercase tracking-[0.24em] text-primary">FRIEND CHALLENGE</p><h1 className="mt-3 text-3xl font-bold">Beat {link[0].score} points</h1><p className="mt-2 text-muted-foreground">Your friend has challenged you. Start a run to take the lead.</p><a href="/" className="mt-6 block rounded-xl bg-primary px-4 py-3 font-semibold text-primary-foreground">Play challenge</a></div></main>
}
